	...
	<div class="container">
		<div class="row">
			<div class="col-md-8">
				<div class="row">
				<?php					
					$searchDate = date('m-Y',strtotime($_GET['id']));
					...
					$total_rows = table_row_count('news', 'published_at <="'. $today .'" AND category ="'. $searchDate .'" AND status=1');
					$total_pages = ceil($total_rows / $limit);
					$newses = table_fetch_rows('news', 'published_at <="'. $today .'" AND category ="'. $searchDate .'" AND status=1', 'published_at DESC', $start, $limit);
					if(count($newses)){
						foreach($newses as $news){
							$news['published_at'] = date('M d Y',strtotime($news['published_at']));
							$newsDate = date('m-Y',strtotime($news['published_at']));
				...
				?>
					<div class="col-lg-6 col-sm-6 bottom_margin">
						<div class="news_list">
							<div class="news_item">
				...
				<?php
					...
					{
						$searchDate = date('F Y',strtotime($_GET['id']));
						echo "There is no news post found in $searchDate.";
					}
				?>		
