<?php
    ...
    */
    ...
    if(isset($_POST['save']))
    {
        ...
        if($_POST['id'] == 0) 
        {
            $table_id = table_insert('team', $fields, $_POST);
            $messages[] = 'Saved successfully.';
           
        }
        else 
        {
            table_update('team', $fields, $_POST, 'id=' . get_id());
            $messages[] = 'Saved successfully.';
        }   
    	...
    	if(isset($_FILES['image']) && !empty($_FILES['image']['tmp_name'])) {
    		...
    		$image->toFile(UPLOADS_DIR . 'team/' .$table_id . '-team-member.' . $imgData['extension']);
        ...
        if(isset($_FILES['meta-image']) && !empty($_FILES['meta-image']['tmp_name'])) {
            ...
            $meta_image->toFile(UPLOADS_DIR . 'team/' .$table_id . '-meta-image.' . $imgData['extension']);

        }
        saveRewrite('team',$table_id,'',$_POST['url']);
  
    }
     
    if($table_id > 0) {
        $data = table_fetch_row('team', 'id=' . $table_id); 
        if($data !== false) {
        	$data['url'] = getRewriteUrl('team', $data['id']);
        }
    }
...
<form class="validate-form" method="post" enctype="multipart/form-data">
    ...
    <div class="card mb-4">
        <div class="card-header">
            <h1><?php echo ($table_id == 0) ? 'Add' : 'Edit'; ?> Team Member</h1>
        </div>
        <div class="card-body">
            ...
            <div class="form-group">
                <?php    
                if($table_id > 0) {  
                    $path = get_image('team/' . $data['id'] . '-team-member');

                    if (strlen($path) > 0):
                        ?>
               
                        <?php show_image('team/' . $data['id'] . '-team-member'); ?>
...
<!--             <div class="form-group">
                ...
                <input class="required form-control" name="job_title" id="job_title" size="50" type="text" value="<?php echo isset($data['job_title']) ? $data['job_title'] : ''; ?>" />
            </div> -->
            <div class="form-group">
...
<!--             <div class="form-group">
                ...
                <input class="required form-control" name="telephone" id="telephone" size="50" type="text" value="<?php echo isset($data['telephone']) ? $data['telephone'] : ''; ?>" />
            </div>  -->
<!--             <div class="form-group">
                <label for="mobile">Mobile (for Whatsapp):</label>
                <input class="required form-control" name="mobile" id="mobile" size="50" type="text" value="<?php echo isset($data['mobile']) ? $data['mobile'] : ''; ?>" />
            </div>
            <div class="form-group">
                <label for="email">Email:</label>
                <input class="required form-control" name="email" id="email" size="50" type="text" value="<?php echo isset($data['email']) ? $data['email'] : ''; ?>" />
            </div>  --> 
            <div class="form-group">
