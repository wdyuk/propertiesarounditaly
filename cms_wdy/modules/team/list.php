<?php
	...
	$total_rows = table_row_count('team');
	$total_pages = ceil($total_rows / $limit);
	
	$rows = table_fetch_rows('team', '', 'position ASC', ($page-1) * $limit, $limit);
        ...
        <div class="form-row align-items-center">
        	<table class="list">
				...
				<?php 
					...
					show_rows($rows, 'team', array('name','job_title','status'), $operations);
				?>
				</tbody>
				<tfoot>
				<tr>
...
$(function() {
	$('table.list > tbody').sortable({
                                            ...
                                            stop: function (evt, ui) {
                                                ...
                                                var params = 'table=team&' + arr.join('&');
	...
	$('.operation').live('click', function() {
		...
		if (elm_class.indexOf('operation-form') > -1) {
			var location = 'control-panel.php?module=team&action=form&id=' + value;
			window.location = location;
		} else if (elm_class.indexOf('operation-delete') > -1) {
			var $li = $(this).parents('tr');
			
			if (confirm('Are you sure you want to delete this?')) {
				$.post('ajax/delete.php', 'id=' + value + '&table=team', function() {
					$li.remove();
				});
			}
		}
