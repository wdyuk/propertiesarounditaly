<?php
	...
	$sql = 'is_sold = 0';
	//$sort_by = 'type_vehicle asc, make asc, model asc';
	$sort_by = 'id asc';
	if(!empty($_POST))
	{
		$sql = '';
		...
		// 	$sql .= "(LOWER(variant) LIKE '%$key%' OR
		// 	LOWER(meta_keywords) LIKE '%$key%') AND ";
		// }
		// $sql = rtrim($sql,' AND ');
...
<div class="card mb-4">
    <div class="card-header">
        Vehicles
    	<br/>
    	Total Vehicles for Sale: <?php echo $total_rows;?>
    </div>
    <div class="card-body">
			...
			<table class="list">
				...
				<tbody class="cursor-move">
					<?php 
						...
						show_rows($rows, 'vehicles', array('registration','title','make','model','price','is_sold','category'), $operations);
					?>
				</tbody>
				<tfoot>
					<tr>
...
$(function() {
	...
	$('table.list > tbody').sortable({
                                            ...
                                            stop: function (evt, ui) {
                                                ...
                                                var params = 'table=cars&' + arr.join('&');
	...
	$('.operation').live('click', function() {
		...
		if (elm_class.indexOf('operation-form') > -1) {
			var location = 'control-panel.php?module=cars&action=form&id=' + value;
			window.location = location;
		} else if (elm_class.indexOf('operation-delete') > -1) {
			var $li = $(this).parents('tr');
			
			if (confirm('Are you sure you want to delete this?')) {
				$.post('ajax/delete.php', 'id=' + value + '&table=cars', function() {
					$li.remove();
				});
			}
		}
