...
function get_table_photos($tablename, $tableid, $type, $keys = true)
{
    ...
    if(count($data) > 0)
    {
        foreach($data as $counter => $photo)
        {
            $file = get_image('module_images/' . $photo['groupings'] . '-FULL');
            ...
            if($file !== false)
            {
				...
				$photos[$photo['id']]['slider_alt'] = $photo['slider_alt'];
				$photos[$photo['id']]['banner_text'] = $photo['banner_text'];
